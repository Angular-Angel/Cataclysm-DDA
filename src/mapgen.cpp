...
void map::generate(const int x, const int y, const int z, const int turn)
{
    ...
    complex_map_tile terrain_tile = overmap_buffer.ter(overx, overy, z);
    ...
    for (unsigned int i = 0; i < terrain_tile.tiles.size(); i++) {
        draw_map(terrain_tile.tiles[i], terrain_tile, t_north, t_east, t_south, t_west, t_neast, t_seast, t_nwest, t_swest,
                 t_above, turn, density, z, rsettings);
    }

    map_extras ex = get_extras(terrain_tile.extras());
    if ( one_in( ex.chance )) {
        add_extra( random_map_extra( ex ));
    }

    const overmap_spawns &spawns = terrain_tile.static_spawns();
...
void map::draw_map(const oter_id terrain_type, const complex_map_tile terrain_tile, 
